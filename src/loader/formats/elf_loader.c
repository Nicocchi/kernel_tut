...
static bool efl_valid_signature(void* buffer)
{
...
{
    return header->e_type == ET_EXEC && header->e_entry >= PANDORA_PROGRAM_VIRTUAL_ADDRESS;
}
...
// Return program header based on index
struct elf32_phdr* elf_program_header(struct elf_header* header, int index)
{
    return &elf_pheader(header)[index];
}

// Return section header based on index
