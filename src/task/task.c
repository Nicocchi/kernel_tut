...
// Needs to be called from kernel land
...
void* task_get_stack_item(struct task* task, int index)
{
    ...
    // Making an array to the stack pointer to the task - virt addr, not phys
    uint32_t* sp_ptr = (uint32_t*) task->registers.esp;

    task_page_task(task); // Switch to the given tasks page
...
{
    return paging_get_phyiscal_address(task->page_directory->directory_entry, virtual_address);
}