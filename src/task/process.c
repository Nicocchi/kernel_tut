...
static struct process* processes[PANDORA_MAX_PROCESSES] = {}; // ALl running processes
...
{
    if (process_id < 0 || process_id >= PANDORA_MAX_PROCESSES)
    {
        return NULL;
    }
...
static int process_terminate_allocations(struct process* process)
{
    for (int i = 0; i < PANDORA_MAX_PROGRAM_ALLOCATIONS; i++)
    {
        ...
        {
            process_free(process, process->allocations[i].ptr); // could just do a kfree on the ptr
        }
    }
...
{
    for (int i = 0; i < PANDORA_MAX_PROCESSES; i++)
    {
        if (processes[i])
        {
    ...
    panic("process_switch_to_any: No processes to switch too\n");
}
...
int process_free_process(struct process* process)
{
    // Might be better to panic if these return an error
    int res = 0;
...
// Injects the arguments into the process
...
{
    int res = 0;
    paging_map_to(process->task->page_directory, (void*)PANDORA_PROGRAM_VIRTUAL_ADDRESS, 
...
// Maps the memory to the paging table
int process_map_memory(struct process* process)
{
    ...
    // Map the stack
    paging_map_to(process->task->page_directory, (void*)PANDORA_PROGRAM_VIRTUAL_STACK_ADDRESS_END, process->stack, paging_align_address(process->stack + PANDORA_USER_PROGRAM_STACK_SIZE), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
...
int process_load_for_slot(const char* filename, struct process** process, int process_slot)
{
    ...
    _process->stack = kzalloc(PANDORA_USER_PROGRAM_STACK_SIZE);
    if (!_process->stack)
    {
        res = -ENOMEM;
        goto out;
    ...
    if (ERROR_I(_process->task) == 0)
    {
        ...
        _process->task = NULL; // NULL due to err code returned from task_new()
        goto out;
    }
