...
#define PANDORA_FAT16_SIGNATURE         0x29
#define PANDORA_FAT16_FAT_ENTRY_SIZE    0x02
#define PANDORA_FAT16_BAD_SECTOR        0xFF7
#define PANDORA_FAT16_UNUSED            0x00

// These 3 lines don't represent anyting on the disk
...
struct fat_directory_item
{
    ...
    uint16_t low_16_bits_first_cluser;
    uint32_t filesize;
} __attribute__((packed));

struct fat_directory
{
    struct fat_directory_item* item; // Points to the first item in the directory
    int total; // Total number of items
    int sector_pos; // First sector the data is in
    int ending_sector_pos; // Last sector where this data is stored
};
...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector)
{
    ...
    if (diskstreamer_seek(stream, directory_start_pos) != PANDORAOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    while(1)
    {
        if (diskstreamer_read(stream, &item, sizeof(item)) != PANDORAOS_ALL_OK)
        {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory)
{
    ...
    int root_dir_sector_pos = (primary_header->fat_copies * primary_header->sectors_per_fat) + primary_header->reserved_sectors; // The first sector
    ...
    struct disk_stream* stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != PANDORAOS_ALL_OK)
    {
        res = -EIO;
        goto err_out;
    }

    if (diskstreamer_read(stream, dir, root_dir_size) != PANDORAOS_ALL_OK)
    {
        res = -EIO;
        goto err_out;
    }
...
int fat16_resolve(struct disk* disk)
{
    ...
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != PANDORAOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != PANDORAOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
// Looks for a space or null terminator and if there is a space
// adds a null terminator.
// [FILE    ] -> 8 bytes because a filename is 8 bytes, and ends up
// being [FILE]
void fat16_to_proper_string(char** out, const char* in, size_t size)
{
    int i = 0;
    // Checks for null terminator or space
...
// test ext = txt
// returns test.txt into tmp_filename
...
{
    return (item->high_16_bits_first_cluster << 16) | item->low_16_bits_first_cluser;
}

// Takes us directory to the cluster the directory represents
// Read the manual why we need to minus 2 to the cluster, but essentially in the File Allocation Table when we want to
// convert the clusters to sectors we need to minus 2 to it
...
// The very first File Allocation Table comes directly after the reserved sectors
...
static int fat16_get_fat_entry(struct disk* disk, int cluster)
{
    ...
    res = diskstreamer_seek(stream, fat_table_position + (cluster * PANDORA_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
    {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < clusters_ahead; i++)
    {
        ...
        // Sector is bad
        if (entry == PANDORA_FAT16_BAD_SECTOR)
        {
            res = -EIO;
            goto out;
        }
...
// Will take the starting_cluster and read x amount (total) of bytes from that cluster
// If it exceeds the cluster size, it'll look in the File Allocation Table for the next cluster and
// read from that one. It'll make it look like we are reading from one giant file.
...
struct fat_directory* fat16_load_fat_directory(struct disk* disk, struct fat_directory_item* item)
{
    ...
    int cluster_sector = fat16_cluster_to_sector(fat_private, cluster); // Sector number of the cluster
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name)
{
    ...
    char tmp_filename[PANDORA_MAX_PATH];
    for (int i = 0; i < directory->total; i++)
    {
        ...
        {
            // Found it and create a new fat_item
...
struct fat_item* fat16_get_directory_entry(struct disk* disk, struct path_part* path)
{
    ...
    // 0:/test.txt -> path->part would be test.txt and return a fat_item for test.txt
    // 0:/abc/test.txt -> the root item would be abc
