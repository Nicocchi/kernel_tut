...
{
    int len = strnlen(filename, PANDORAOS_MAX_PATH);
    // Gets the address of the first index on the filename and checks if the two bytes are :/
    return (len >= 3 && isdigit(filename[0]) && memcmp((void*)&filename[1], ":/", 2) == 0); // Valid -> 0:/ Not valid -> A:/
}

// Goes from 0:/test.txt to test.txt
...
{
    char* result_path_part = kzalloc(PANDORAOS_MAX_PATH);
    if (!result_path_part)
    {
        return NULL;
    }
...
// Create the path part structure elements
...
struct path_root* pathparser_parse(const char* path, const char* current_directory_path)
{
    ...
    if (strlen(path) > PANDORAOS_MAX_PATH)
    {
        res = -1;
        goto out;
    }
